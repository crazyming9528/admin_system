  ...
  <ul>
    <template v-for="(item,index) in listData">
      <li v-if="item.child.length <= 0" :class="{'primary-menu':item.type==='primary'}">
        <a href="javascript:void(0)"><input type="text" :value="item.title"></a>
      ...
      <li v-else :class="{'primary-menu':item.type==='primary'}" >
        <a href="javascript:void(0)" class="color-blue" @click="showChildMenu(index)"><input type="text" :value="item.title"></a>
        <div class="btn-group" v-if="item.type!=='primary'">
